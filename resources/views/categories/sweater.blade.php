...
@section('main')
    <div class="page-title">
        <h1 class="font-semibold text-4xl uppercase">sweater</h1>
    </div>
        ...
        <div class="p-8">
            <div class="flex shadow-lg rounded-lg p-6">
                <div class="project-img">
                    <img src="../assets/sweater{{ $loop->iteration }}.png" alt="" />
                </div>
                <div class="relative w-full py-4">
                    <h3 class="text-xl font-normal">{{ $sweater->nama }}</h3>
                    <p>Rp{{ $sweater->harga }}</p>
                    <div class="absolute right-0 top-0 bottom-0 flex flex-col justify-center px-6 py-10 gap-3">
                        @if ($sweater->s)
                            <button onclick="size('s-{{ $sweater->id }}')"
                        ...
                        @if ($sweater->m)
                            <button onclick="size('m-{{ $sweater->id }}')"
                        ...
                        @if ($sweater->l)
                            <button onclick="size('l-{{ $sweater->id }}')"
                        ...
                        @if ($sweater->xl)
                            <button onclick="size('xl-{{ $sweater->id }}')"
                    ...
                    <button onclick="addToKeranjang({{ $sweater->id }})"
