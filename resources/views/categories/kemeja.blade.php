...
@section('main')
    <div class="page-title">
        <h1 class="font-semibold text-4xl uppercase">kemeja</h1>
    </div>

    @foreach ($kemejas as $kemeja)
        <div class="p-8">
            <div class="flex shadow-lg rounded-lg p-6">
                <div class="project-img">
                    <img src="../assets/kemeja{{ $loop->iteration }}.png" alt="" />
                </div>
                <div class="relative w-full py-4">
                    <h3 class="text-xl font-normal">{{ $kemeja->nama }}</h3>
                    <p>Rp{{ $kemeja->harga }}</p>
                    <div class="absolute right-0 top-0 bottom-0 flex flex-col justify-center px-6 py-10 gap-3">
                        @if ($kemeja->s)
                            <button onclick="size('s-{{ $kemeja->id }}')"
                        ...
                        @if ($kemeja->m)
                            <button onclick="size('m-{{ $kemeja->id }}')"
                        ...
                        @if ($kemeja->l)
                            <button onclick="size('l-{{ $kemeja->id }}')"
                        ...
                        @if ($kemeja->xl)
                            <button onclick="size('xl-{{ $kemeja->id }}')"
                    ...
                    <button onclick="addToKeranjang({{ $kemeja->id }})"
